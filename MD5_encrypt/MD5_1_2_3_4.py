"""
这个是第四种优化：解决混沌函数出现常数代换表的问题
由于对logistic生成的代换表的操作是以相邻的两位十六进制数为单位进行的，故在logistic结束后，采用henon生成的矩阵和abcd进行按十六进制位相乘，消除相邻两个十六进制数的相关性
henon的生成种子由原文分组的固定位决定
"""
import math
import random_bytes
...
def md5(message):
    ...
    message = cubic_encrypt.cubic_sum(message)  # 优化1
    ...
    for i in range(0, len(message), 64):
        ...
        # 为解决上一步代换表容易翻车的问题，采用非常规矩阵乘法进行处理，同时消除相邻位的相关性
        # 矩阵由henon混沌映射生成，原文内容为种子
        hen = henon_encrypt.generate_henon_array(4, 8, 1.4 - 0.001 * chunk[10], 0.3 - 0.001 * chunk[50], 0, 15)
        # print(hen, "\n\n")  # 显示用 可以删掉

        a = henon_encrypt.multiply_string_item(a, hen[0])
        b = henon_encrypt.multiply_string_item(b, hen[1])
        c = henon_encrypt.multiply_string_item(c, hen[2])
        d = henon_encrypt.multiply_string_item(d, hen[3])
...
"""
# 测试
message_sample = random_bytes.random_bytes_init(512)
result = md5(message_sample)
print("md5+CubicSum+Sbox+log+hen:\t", result)
"""
