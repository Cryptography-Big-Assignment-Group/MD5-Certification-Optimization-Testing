...
def md5(message):
    ...
    for i in range(0, len(message), 64):
        ...
        log = logistic_encrypt.generate_chaos_sequence(0.5457 - chunk[63] * 0.000002 * chunk[32],
                                                       3.9 - chunk[0] * 0.001)
        # print(log, "\n\n")  # 显示用 可以删掉
        a = format(a, 'x').zfill(8)
        b = format(b, 'x').zfill(8)
        c = format(c, 'x').zfill(8)
        d = format(d, 'x').zfill(8)
...
"""
# 测试
message = random_bytes.random_bytes_init(512)
result = md5(message)
print("md5+CubicSum+Sbox+log:\t", result)
"""
